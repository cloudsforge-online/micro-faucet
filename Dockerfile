...
# run time. There is no ENV line here on purpose — least of all CUSTODY_TOKEN, which is what buys a
# signature out of the faucet's funding address and is therefore exactly the thing a leaked image
# layer would hand over. There is no private key to bake in at all: custody holds it.
...
# below SCHEMA_VERSION the partial unique indexes `dispenses_in_flight_uniq` and
# `dispenses_live_recipient_uniq`, and the `faucet_budget_within_cap` CHECK, may not exist. The
# first is what makes two in-flight transactions on one chain impossible. A service that could
# create it at boot is a service that could start without it — two workers signing against one
# nonce with nothing between them. `index.ts` asserts the schema version and refuses to serve below
# it.
